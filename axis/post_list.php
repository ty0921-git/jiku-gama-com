...
if ($keyword) {
  $sql = "select*from $table where title like '%$keyword%' or article like '%$keyword%'";
}
...
if ($keyword) {
  $sql = "select*from $table where title like '%$keyword%' or article like '%$keyword%' order by code DESC limit $start,$disp_data";
}
$stmt = connect()->query($sql);
      ...
      ?>
        <div class="row g-3 mb-5">
          ...
          <div class="col-md-9">
            ...
            <h5><a href="<?= $table ?>_form.php?code=<?= $code ?>"><?= h($row['title']) ?></a></h5>
            <div><?= $article ?></div>
          </div>
          <div class="col-md-1">
