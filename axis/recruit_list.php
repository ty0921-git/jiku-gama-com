...
if ($keyword) {
    $sql = "select*from $table where rec_title like '%$keyword%' or short_description like '%$keyword%'";
}
...
if ($keyword) {
    $sql = "select*from $table where rec_title like '%$keyword%' or short_description like '%$keyword%' order by code DESC limit $start,$disp_data";
}
$stmt = connect()->query($sql);
            ...
            ?>
                ...
                <div class="row g-3 mb-5">
                    ...
                    <div class="col-md-9">
                        ...
                        <?php
                        if ($row['rec_occupation']) {
                            $rec_occupation = " / " . $row['rec_occupation'];
                        } else {
                            $rec_occupation = "";
                        }
                        ?>
                        <h5 class="mb-4"><a href="<?= $table ?>_form.php?code=<?= $code ?>"><?= h($row['rec_title']) ?><?= h($rec_occupation) ?></a></h5>

                        <?php
                        if ($row['date_end'] != "0000-00-00") {
                            $date_end = "<span class=\"bg-cred py-2 px-3 rounded\">掲載終了日 " . $row['date_end'] . "</span>";
                        } else {
                            $date_end = "掲載終了日 設定なし";
                        }
                        ?>
                        <div><?= h($date_end) ?> / 登録日 <?= h($row['date_regi']) ?></div>
                    </div>
                    <div class="col-md-1">
