...
// 設備データ取得
$facis = get_category("facility");

// 設備連想配列の作成
$faci_array = array();
foreach ($facis as $faci) {
    $faci_array += array($faci['code'] => $faci['cate_name']);
}
...
if ($keyword) {
    $sql = "select*from $table where spot_name like '%$keyword%' or description like '%$keyword%' or short_description like '%$keyword%'";
}
...
if ($keyword) {
    $sql = "select*from $table where spot_name like '%$keyword%' or description like '%$keyword%' or short_description like '%$keyword%' order by code DESC limit $start,$disp_data";
}
$stmt = connect()->query($sql);
                ...
                ?>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="mb-3">
                            ...
                            <?php endforeach; ?>
                            <?php
                            $faci_codes = get_chains($table, $code);
                            foreach ($faci_codes as $faci_code) :
                            ?>
                                <span class="badge bg-cblue"><?= h($faci_array[$faci_code]) ?></span>
                            <?php endforeach; ?>
                            <?php
                        ...
                        <div>
                            <h5><a href="<?= $table ?>_form.php?code=<?= $code ?>"><?= h($row['spot_name']) ?></a></h5>
                            <?php if ($row['spot_zip']) : ?>
                                <div>〒<?= h($row['spot_zip']) ?></div>
                            <?php endif; ?>
                            <div>
                                <?php if ($row['spot_add01']) : ?>
                                    <?= h($row['spot_add01']) ?>
                                <?php endif; ?>
                                <?php if ($row['spot_add02']) : ?>
                                    <?= h($row['spot_add02']) ?>
                                <?php endif; ?>
                                <?php if ($row['spot_add03']) : ?>
                                    <?= h($row['spot_add03']) ?>
                                <?php endif; ?>
                            </div>
                            <?php if ($row['spot_tel']) : ?>
                                <div>TEL <?= h($row['spot_tel']) ?></div>
                            <?php endif; ?>
                        </div>
                        <div class="text-end">
